...
const Layout = ({ children, isLoggedIn, handleLogout }) => {
    ...
    const handleLogoClick = (e) => {
        if (isLoggedIn) {
            e.preventDefault();
            const confirmLogout = window.confirm("Are you sure you want to log out and go to landing page?");
            if (confirmLogout) {
        ...
        <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans text-slate-800 selection:bg-slate-200 selection:text-slate-900">
            ...
            >
                <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
                    ...
                    {/* Left: Logo + Brand */}
                    <Link to="/" onClick={handleLogoClick} className="flex items-center gap-3 group outline-none">
